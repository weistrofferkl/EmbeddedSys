...
#define LINE_DELAY XS1_TIMER_HZ/4
 ...
 //   int weirdTurnFlag = 0;
    ...
    if(rightTurnFlag == 1){
        ...
        if(newYaw > (PI2)){
            ...
            while((curYaw < ((2*PI)-.15)) && (curYaw > (.15))){
                dutyCycle.left_duty_cycle = ZERO_DUTY_CYCLE;
                dutyCycle.right_duty_cycle = GOOD_TURN_DUTY_CYCLE;
                out_motor_cmd_chan <: dutyCycle;
                YPR <: 1;
        ...
        if(newYaw < 0 ){
            ...
            sprintf(buffer, "HOLDER YAW: %f\n", holderYaw);
            printstrln(buffer);
            sprintf(buffer, "NEW YAW: %f\n", newYaw);
            printstrln(buffer);

            sprintf(buffer, "CUR YAW: %f\n", curYaw);
            printstrln(buffer);

            sprintf(buffer, "2PI-CURYAW: %f\n", (PI2-curYaw));
            printstrln(buffer);
...
void driveStraightNTicks(chanend out_motor_cmd_chan, chanend encoderChan, chanend YPR, float yawSetting, int n, int Lduty, int Rduty){
    ...
    while(1){
        ...
        if(yawDiff > YAW_THRESHOLD){
            ...
            Lduty = p0L+(1+yawDiff*kp);
            Rduty = p0R-(1+yawDiff*kp);
        }
...
void formatCommand(char* charArray, chanend trigger_chan, chanend out_motor_cmd_chan, chanend encoderChan, chanend tickSig, chanend wifiV, chanend YPR){
   ...
   //Moves Robot forward at 100 and 50 percent dutyCycle:
    ...
    //MODIFIED TO USE YPR VALUES:
        ...
        driveStraightNTicks(out_motor_cmd_chan,encoderChan, YPR,yawSetting, 20, DRIVER_HALF_HIGH, DRIVER_HALF_HIGH);
    ...
    //Turn Right 90 Deg.
        ...
        turn90Deg(out_motor_cmd_chan, 1, YPR);
    }
    //Turn Left 90 Deg.
    else if(strcmp(charArray, ",") == 0){

        turn90Deg(out_motor_cmd_chan, 0, YPR);
    }
